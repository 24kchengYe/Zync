...
`UpdateWuruize` 的定位是增量说明目录：

- 当前只保留更新说明文档，不再存放正式运行依赖的 TypeScript / React 源码。
- 真正运行的 i18n、workspace layout、snapshot、activity stream 等模块已迁入 `frontend/src/`。
...
- 已交付：
  ...
  - 动作入口当前统一标记为 `Preview / 预览`，并保持禁用，避免在没有后端执行链路时展示虚假的执行成功或排队状态。
- 当前边界：
  - 当前只是前端交互原型。
  - 当前不再显示批量执行结果卡，也不会生成 `success / queued` 等伪结果。
