...
export function registerSessionHandlers(ipcMain: IpcMain, services: AppServices): void {
  ...
  ipcMain.handle('sessions:permanently-delete', async (_event, sessionId: string) => {
    try {
      const dbSession = databaseService.getSession(sessionId);
      if (!dbSession) {
        return { success: false, error: 'Session not found' };
      ...
      const result = databaseService.permanentlyDeleteSession(sessionId);
      if (result) {
        console.log(`[Session IPC] Permanently deleted session ${sessionId}`);
        // Notify renderer via BrowserWindow if available
