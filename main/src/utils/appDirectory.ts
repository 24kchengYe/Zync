...
/**
 * Sets a custom Zync directory path. This should be called early in the
...
/**
 * Determines if Zync is running from an installed application (DMG/Applications folder)
...
/**
 * Gets the Zync directory path. Returns the custom directory if set,
...
export function getAppDirectory(): string {
  ...
  if (isInstalledApp()) {
    console.log('[Zync] Running as installed app, using ~/.pane');
    return join(homedir(), '.pane');
  }

  // 4. If running inside Zync (detected by bundle identifier) in development, use development directory
  // This prevents development Zync from interfering with production Zync
  if (process.env.__CFBundleIdentifier === 'com.zync.app' && !app.isPackaged) {
    console.log('[Zync] Detected running inside Pane development, using ~/.pane_dev for isolation');
...
export function migrateDataDirectory(): void {
  ...
  if (!existsSync(newDir) && existsSync(oldDir)) {
    try {
      ...
      console.log(`[Zync] Migrated data directory: ${oldDir} → ${newDir}`);
    } catch (err) {
      console.error(`[Zync] Failed to migrate data directory: ${err}`);
    }
  }
  ...
  if (!existsSync(newDevDir) && existsSync(oldDevDir)) {
    try {
      ...
      console.log(`[Zync] Migrated dev directory: ${oldDevDir} → ${newDevDir}`);
    } catch (err) {
      console.error(`[Zync] Failed to migrate dev directory: ${err}`);
    }
  }
}

/**
 * Gets a subdirectory path within the Zync directory
 */
