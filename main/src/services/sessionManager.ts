      ...
      for (const panel of panels) {
        if (panel.type === 'terminal') {
          ...
          if (termState?.wasInterrupted && termState?.initialCommand) {
            ...
            // Re-launch claude without session ID injection — let Claude Code manage its own sessions
            const skipPerms = dbSession.permission_mode !== 'approve';
            customState.initialCommand = skipPerms ? 'claude --dangerously-skip-permissions' : 'claude';
