...
export class TerminalPanelManager {
  ...
  async initializeTerminal(panel: ToolPanel, cwd: string, wslContext?: WSLContext | null, priority: number = 1): Promise<void> {
    ...
    let enhancedPath = isLinux ? (process.env.PATH || '') : getShellPath();
    // Remove local node_modules/.bin from PATH so global claude is used
    const sep = process.platform === 'win32' ? ';' : ':';
    enhancedPath = enhancedPath.split(sep).filter(p => !p.includes('node_modules')).join(sep);
    ...
    if (initialCommand) {
      ...
      // Let Claude Code manage its own sessions natively
      // No --session-id or --resume injection — user's recall skill and
      // Claude Code's built-in project memory handle conversation persistence
