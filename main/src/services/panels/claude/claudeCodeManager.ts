...
export class ClaudeCodeManager extends AbstractCliManager {
  ...
  protected buildCommandArgs(options: ClaudeSpawnOptions & { mcpConfigPath?: string | null }): string[] {
    ...
    if (effectiveMode === 'ignore') {
      ...
      // IPC server not available, use Claude Code's built-in permission prompts
      // by simply not adding --dangerously-skip-permissions
      this.logger?.verbose(`Using Claude Code's built-in permission mode (IPC server not available)`);
    }
