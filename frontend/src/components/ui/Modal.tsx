...
export const Modal: React.FC<ModalProps> = ({
  ...
  const handleOverlayClick = (e: React.MouseEvent) => {
    ...
    // Check if click is inside a portal dropdown (rendered on document.body with high z-index)
    const isClickInsidePortal = e.target instanceof HTMLElement && !!e.target.closest('[class*="z-[10000]"]');

    // Only close if:
    // 1. closeOnOverlayClick is enabled
    // 2. The click is not inside the modal content
    // 3. The click is not inside a portal dropdown
    // 3. The mousedown also started outside the modal content
    if (closeOnOverlayClick && !isClickInsideModal && !isClickInsidePortal) {
      ...
      const wasMouseDownInsidePortal = mouseDownTargetRef.current instanceof HTMLElement && !!mouseDownTargetRef.current.closest('[class*="z-[10000]"]');
      if (!wasMouseDownInsideModal && !wasMouseDownInsidePortal) {
        onClose();
      }
    }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        tabIndex={-1}
        className={cn(
          'relative bg-bg-primary rounded-modal shadow-modal w-full max-h-[90vh] flex flex-col',
          sizeClasses[size],
          'animate-fadeIn',
          className
        )}
