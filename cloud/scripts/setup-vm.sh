...
if [ ! -f /usr/bin/Pane ]; then
  ...
  RELEASE_URL=$(curl -fsSL https://api.github.com/repos/24kchengYe/Zync/releases/latest \
  ...
  if [ -n "${RELEASE_URL}" ] && [ "${RELEASE_URL}" != "null" ]; then
    ...
    DEB_URL=$(curl -fsSL https://api.github.com/repos/24kchengYe/Zync/releases/latest \
